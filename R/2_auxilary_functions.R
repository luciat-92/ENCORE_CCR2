...
create_input_list <- function(model_dual,
  ...
  result_file <- sprintf("%s/%s_FINAL_EXACT_logFC_sgRNA.txt", fold_input_dual, tissue)
  names(result_file) <- "result_file"
  if (tissue == "COLO") {
    library_file <- sprintf("%s/../ENCORE_GI_COREAD_Library_ALL.txt", fold_input_dual)
  }
  if (tissue == "BRCA") {
    library_file <- sprintf("%s/../ENCORE_GI_BRCA_Library_ALL.txt", fold_input_dual)
  }
